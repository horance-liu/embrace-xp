...
万事开头难，第一个用例跑起来并不容易。此处设计实现了一个简单的测试用例，用户通过扩展框架的\ascii{TestCase}，实现测试用例的新增。一般地，在面向对象设计中，通常通过扩展子类实现是实现开放封闭原则的重要手段。
...
在\ascii{TestCase}中，直接使用\ascii{using}语句复用构造函数。
