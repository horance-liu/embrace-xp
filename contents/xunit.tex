...
在之前的测试用例里，在匿名命名空间内引入计数器\ascii{num}。当\ascii{TestCase}被执行时，其\ascii{num++}。显然，这是一种临时的设计方案，用户需要小心地维护计数器的初始化，及其控制累加计数器的时机。这是一种脆弱的设计，一则容易引入不经意的错误，二则让计数器的操作散乱到各个覆写的\ascii{TestCase::runTest}之中。
...
\ascii{TestResult}维护了一个计数器，负责计数器初始化，累计操作等职责。
...
为了通过测试，当执行\ascii{TestCase::run}时，通知\ascii{TestResult}累加计数器。为了使得代码具有层次感，提取了\ascii{TestCase::runBare}的子函数。
