...
本章重点讨论用例组织的实现方式。通过引入\emph{树结构}，提供了一种处理相似逻辑的一般性方法，极大地简化了客户组织用例与运行用例的代码逻辑。最后，为了区分每个测试用例，为其增加命名机制。
...
为了快速通过编译，创建\ascii{TestSuite}的头文件。需要注意的是，\ascii{TestSuite}仅对\ascii{TestCase}的指针类型产生依赖，前置声明\ascii{struct TestCase}即可。但是，\ascii{TestSuite}对\ascii{std::vector}则表现为强依赖，因此需要包含头文件。
...
\ascii{TestSuite}持有若干\ascii{TestCase}实例，引入析构函数释放动态添加的\ascii{TestCase}实例。
